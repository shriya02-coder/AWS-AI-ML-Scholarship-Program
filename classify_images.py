...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic: 
       # Using classifier function to classify images and obtain model label
       model_label = classifier(images_dir + key, model)
       
       # Convert the model label to lowercase and strip whitespace
       clean_label = model_label.lower().strip()
              
       # Pet image label
       pet_label = results_dic[key][0]

       # Check for a match and update results dictionary
       if pet_label in clean_label:
           results_dic[key].extend([clean_label, 1])
       else:
           results_dic[key].extend([clean_label, 0])
           print(f"No match for {key}. Updated results: {results_dic[key]}")
