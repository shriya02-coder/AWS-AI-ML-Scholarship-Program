...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        # Processes each line in file until reaching EOF by processing line and adding dognames to dog_names_dict with while loop
        while line != "":
            # Remove the newline character from the line
            line = line.strip()
            
            # Adds dogname(line) to dog_names_dict if it doesn't already exist 
    ...
    for key in results_dic:
        # Determine if the pet image label is a dog
        is_pet_dog = 1 if results_dic[key][0] in dog_names_dict else 0
        # Determine if the classifier label is a dog
        is_classifier_dog = 1 if results_dic[key][1] in dog_names_dict else 0
        # Append the results to the list in results dictionary
        results_dic[key].extend((is_pet_dog, is_classifier_dog))

        # Collect data for the table
        table_data.append([key, results_dic[key][0], results_dic[key][1], is_pet_dog, is_classifier_dog])

    # Print the table
    print(tabulate(table_data, headers=["Filename", "Pet Label", "Classifier Label", "Pet is-a-dog", "Classifier is-a-dog"], tablefmt="grid"))